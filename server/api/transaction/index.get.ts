...
export default defineEventHandler(async (events) => {
    try {
        ...
        if (view === 'All') {
            const all = await transactions.find({ user: usersData!._id }).sort({ createdAt: -1 })
            return {
                statusCode: 200,
                current: all,
                last: []
        ...
        const current = await transactions.find({ user: usersData!._id }).sort({ createdAt: -1 }).gte('createdAt', currentPeriode().start).lte('createdAt', currentPeriode().end).gte('createdAt', currentPeriode().start)
        const last = await transactions.find({ user: usersData!._id }).sort({ createdAt: -1 }).gte('createdAt', lastPeriode().start).lte('createdAt', lastPeriode().end)
        return {
            statusCode: 200,
            current,
            last
